...
// write code here
const dataTxt = json.data;

// get how many times age=32 from the data
const data = dataTxt.split("age=32").length - 1;
console.log(data, "times age=32");

//get all the key= from the data string
const keys = dataTxt.match(/key=\w+/g);
console.log(keys?.length, "keys");

//get all the text after = from the keys
...
fs.outputFile(filePath, text, (err) => {
  if (err) return console.error(err);
  console.log("Text written successfully with callbacks!");
});
